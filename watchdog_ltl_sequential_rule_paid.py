...
- If the **robot moved** or the **scene clearly changed** (e.g. different viewpoint, object clearly in a different place, or clearly not in the previous place (no occlusion and no object)), use **current evidence** and the initial reference to decide: the object may have moved, so set False only when you have evidence it is no longer there or is elsewhere.
...
1. **Persistence Rule**
   - If a predicate was True at T-1, it remains True at T
     unless explicitly contradicted BUT THAT DOESNT MEAN YOU SHOULD IMMEDIATELY TAKE PREDICATES IN T-1, MAKE SURE THAT THINGS MATCH TO SOME POINT, LIKE POT STILL ON STOVE.
