...
class DiffusionPolicyInference():
    ...
    def rollout_policy(self, n_obs_steps=2):
        ...
        while not self.stop_inf and step < self.max_steps:
            ...
            # 1. Check if Monitor screamed "STOP" recently
            ...
            ## save for video setup later
            # imgs1=obs_dict['agentview_rgb']
            # imgs2=obs_dict['eye_in_hand_rgb']
            # img=np.concatenate([imgs1, imgs2], axis=1).transpose(1,2, 0)
            # images.append(img)
            ...
            for action in action_pred[:4]:
                if self.stop_inf or self.watchdog.should_stop():
                    self.stop_inf = True
                    break
