...
def decide_recovery(
    ...
    else: # no know state go to reset and infer there 
                ...
                if r_error:
                    ...
                    try:
                        ...
                        if result.status == "violation":
                            ...
                            ### check if its is due to grasped being 1 and no grasped idx is one it can be due to delay. 
                            ## let it roll once more
                            grasped_idx = [7,8,9]
                            print("if any grasped predicate is 1", any(pred_vector[i] == 1 for i in grasped_idx))
                            if r_grasped and not any(pred_vector[i] == 1 for i in grasped_idx):
                                print(f"r_grasped: {r_grasped} and pred_vec {pred_vector} has none of the grasped as true")
                                if grasp_recovery:
                                    print("not first time meaning the robot didnt actaully grasp anything")
                                else:
                                    print("FIrst time lets wait for next prediciton")
                                    grasp_recovery = 1
                                    continue
                            ...
                            if action == RecoveryAction.RECOVER_TO_MODE:
                                ...
                                set_exec_visual_state(ExecVisualState.RECOVERING)
                                ## if state NONE, go to reset and get the state there
                                cmd_pub_socket.send_pyobj({"command": "RESET_POSE"})
...
if __name__ == "__main__":
    ...
    gui_status = threading.Thread(target=exec_status_light, daemon=True).start()
