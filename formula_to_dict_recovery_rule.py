...
class LTLMonitor:
    def __init__(self):
        ...
        formula_list = [line.strip() for line in formula.strip().split('&')]
        transitions = [f for f in formula_list if "-> X" in f]
        self.done_ap = self.get_initial_and_final_nodes(transitions)
        print("self.done_ap: ", self.done_ap )
        
        ## gt start and end from first line in formual
        # print("self.done_ap: ", self.done_ap )
        
    def get_initial_and_final_nodes(self, formulas):
        lhs_nodes = set()
        rhs_nodes = set()
        final_nodes = set()

        pattern = re.compile(r'G\((\w+)\s*->\s*X\(([^)]+)\)\)')
        ...
        for f in formulas:
            ...
            if not match:
                continue
            lhs = match.group(1)
            rhs = {x.strip() for x in match.group(2).split('|')}

            lhs_nodes.add(lhs)
            rhs_nodes.update(rhs)

            # Final node: only transitions to itself
            if rhs == {lhs}:
                final_nodes.add(lhs)
